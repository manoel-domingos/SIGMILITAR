...
export default function DrePage() {
  ...
  return (
    ...
    {showContextModal && (
      <div
        className="fixed inset-0 bg-white/70 dark:bg-slate-900/70 backdrop-blur-sm z-[9999] flex items-center justify-center p-4"
        onMouseDown={(e) => { if (e.target === e.currentTarget) setShowContextModal(false); }}
      >
        <div className="bg-white dark:bg-[#181A20]  border border-[#2B2C33]/10 dark:border-white/10  rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center space-y-5">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm text-[#2B2C33]/80 dark:text-slate-300 ">Voce pode alternar a qualquer momento pelo botao Trocar Escola.</p>
          </div>
          <div className="flex flex-col gap-2 pt-1">
            <button
              onClick={() => { setActiveSchoolContext('DRE'); setShowContextModal(false); }}
              className="w-full py-3 rounded-xl bg-[#0052CC] dark:bg-blue-600 hover:bg-[#0052CC] dark:hover:bg-blue-700 dark:bg-blue-600 text-white text-sm font-semibold transition-colors flex items-center justify-center gap-2"
            >
            ...
            {contextSchools.map(s => (
              <button
                ...
                className="w-full py-3 rounded-xl border border-[#2B2C33]/10 dark:border-white/10  text-sm font-semibold text-[#2B2C33] dark:text-slate-100  hover:bg-[#F4F5F7] dark:hover:bg-[#22252D] dark:bg-[#0F1115] transition-colors flex items-center justify-center gap-2"
              >
