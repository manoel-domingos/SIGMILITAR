...
import { 
  ...
  Zap, BarChart2, Award, AlertTriangle, Info, Gavel, 
  Bot, TrendingUp, Shield, Users, CheckCircle, X, LogIn, Star, Trophy
} from 'lucide-react';
...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    document.title = "SIGMILITAR | Gestão Disciplinar de Alto Desempenho";
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      document.querySelectorAll('.hero-card').forEach((card) => {
        ...
        const depth = parseFloat(htmlCard.getAttribute('data-depth') || '1');
        const baseX = parseFloat(htmlCard.getAttribute('data-base-x') || '0');
        const baseY = parseFloat(htmlCard.getAttribute('data-base-y') || '0');
        const baseZ = parseFloat(htmlCard.getAttribute('data-base-z') || '0');
        
        htmlCard.style.transform = `translateX(${baseX + x * depth}px) translateY(${baseY + y * depth}px) translateZ(${baseZ}px) rotateY(${-15 + x * 0.5}deg) rotateX(${10 - y * 0.5}deg)`;
      });
    };
  ...
  return (
    <div className="bg-[#050505] text-white overflow-x-hidden min-h-screen antialiased scroll-smooth font-sans">
      <style>{`
        ...
        /* TOKENS EXTRAÍDOS DO DESIGN SYSTEM */
        :root {
      ...
      <nav className="ds-nav">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8 text-sm font-medium text-white/60">
            <a href="#problema" onClick={(e) => handleSmoothScroll(e, '#problema')} className="hover:text-white transition-colors">Problema</a>
            <a href="#solucao" onClick={(e) => handleSmoothScroll(e, '#solucao')} className="hover:text-white transition-colors">Solução</a>
            <a href="#modulos" onClick={(e) => handleSmoothScroll(e, '#modulos')} className="hover:text-white transition-colors">Módulos</a>
            <a href="#diferenciais" onClick={(e) => handleSmoothScroll(e, '#diferenciais')} className="hover:text-white transition-colors">Diferenciais</a>
          </div>
          <div className="flex items-center gap-3">
            <Link href="/login" className="flex items-center gap-2 border border-white/20 text-white hover:bg-white/5 hover:border-white px-5 py-2 rounded-full text-sm font-medium transition-all duration-300">
              Acessar Sistema
              <LogIn className="w-4 h-4" />
            </Link>
            <a href="#cta" onClick={(e) => handleSmoothScroll(e, '#cta')} className="hidden sm:flex items-center gap-2 bg-white text-black px-5 py-2 rounded-full text-sm font-medium hover:bg-[#3b82f6] hover:text-white transition-all duration-300">
      ...
      <section id="hero" className="relative min-h-[100vh] flex items-center justify-center overflow-hidden pt-20">
        <div className="particles-container">
          <div className="particle w-1 h-1" style={{ top: '20%', left: '10%', ['--dx' as any]: '60px', ['--dy' as any]: '-80px', animationDelay: '0s', animationDuration: '5s' }}></div>
          <div className="particle w-1.5 h-1.5" style={{ top: '40%', left: '80%', ['--dx' as any]: '-40px', ['--dy' as any]: '-60px', animationDelay: '1s', animationDuration: '7s' }}></div>
          <div className="particle w-1 h-1" style={{ top: '70%', left: '30%', ['--dx' as any]: '50px', ['--dy' as any]: '-70px', animationDelay: '2s', animationDuration: '6s' }}></div>
          <div className="particle w-2 h-2" style={{ top: '60%', left: '70%', ['--dx' as any]: '-30px', ['--dy' as any]: '-90px', animationDelay: '3s', animationDuration: '8s' }}></div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10 grid lg:grid-cols-2 gap-12 items-center w-full">
          ...
          <div className="text-left flex flex-col items-start z-20">
            ...
            <h1 className="text-4xl md:text-6xl lg:text-7.5xl font-semibold tracking-tight text-white mb-6 leading-[1.1] animate-reveal delay-100">
            ...
            <div className="flex flex-col sm:flex-row gap-4 animate-reveal delay-300 w-full sm:w-auto">
              <Link href="/login" className="flex items-center justify-center gap-2 text-white px-8 py-4 rounded-full text-base font-semibold hover:bg-blue-700 transition-colors shadow-[0_0_20px_rgba(37,99,235,0.3)] bg-[#2563eb]">
                Acessar o Sistema
                <LogIn className="w-5 h-5" />
              </Link>
              <a href="#cta" onClick={(e) => handleSmoothScroll(e, '#cta')} className="flex items-center justify-center gap-2 bg-white/5 border border-white/10 text-white px-8 py-4 rounded-full text-base font-medium hover:bg-white/10 transition-colors">
                Agendar Demonstração
              </a>
            </div>

            <div className="mt-16 flex flex-wrap gap-6 md:gap-8 animate-reveal delay-400">
              <div className="flex items-center gap-2 text-sm font-mono text-white/50 uppercase tracking-widest">
          ...
          <div ref={hero3dRef} className="hidden lg:block relative w-full perspective-[1200px]" id="hero3d">
            <div id="hero3d-container" className="w-full flex flex-col gap-4 max-w-[600px] ml-auto relative z-10" style={{ transformStyle: 'preserve-3d' }}>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="hero-card bg-[#050505]/90 backdrop-blur-xl border border-white/10 rounded-2xl p-3 shadow-2xl transition-transform duration-200 ease-out opacity-0 anim-slide-up-2" 
                  ...
                  <div className="flex items-center justify-between border-b border-white/5 pb-2 mb-3 px-2">
                    <span className="text-xs font-mono text-white/50 tracking-wider"># OCORRÊNCIAS</span>
                    <span className="text-[10px] text-gray-400 font-mono tracking-widest">[SYNC]</span>
                  </div>
                ...
                <div className="hero-card bg-[#0A0A0A]/90 backdrop-blur-xl border border-[#2563eb]/30 rounded-2xl p-3 shadow-[0_0_40px_rgba(37,99,235,0.15)] transition-transform duration-200 ease-out opacity-0 anim-slide-up-1" 
                  ...
                  <div className="flex items-center justify-between border-b border-[#2563eb]/20 pb-2 mb-3 px-2 relative">
                    ...
                    <span className="text-xs font-mono text-white/70 tracking-wider"># ARIA_NODE</span>
                    <span className="text-[10px] text-[#3b82f6] font-mono tracking-widest">[INDEX]</span>
                  </div>
              ...
              <div className="hero-card w-full bg-[#0A0A0A]/95 backdrop-blur-xl border border-white/10 rounded-2xl p-3 shadow-2xl transition-transform duration-200 ease-out z-10 opacity-0 anim-slide-down" 
                ...
                <div className="flex items-center justify-between border-b border-white/10 pb-2 mb-3 px-2">
                  <span className="text-xs font-mono text-white tracking-wider"># DASHBOARD_EXECUTIVO</span>
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="flex flex-col md:flex-row items-center gap-16">
            <div className="flex-1 scroll-reveal">
              <span className="text-xs font-mono text-[#2563eb] uppercase tracking-widest mb-4 block">Inteligência Acadêmica</span>
              ...
              <div className="relative bg-black rounded-3xl border border-white/10 p-2 overflow-hidden shadow-2xl">
                ...
                <div className="bg-[#0A0A0A] rounded-2xl border border-white/5 p-6 h-[400px] flex flex-col">
                  <div className="flex justify-between items-center mb-6 border-b border-white/5 pb-4">
                    ...
                    <span className="text-[10px] font-mono text-[#2563eb] border border-[#2563eb]/30 px-2 py-1 rounded">LIVE</span>
                  </div>
      ...
      <section id="modulos" className="relative py-32 border-t border-white/5">
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="text-center max-w-3xl mx-auto mb-16 scroll-reveal">
            <span className="text-xs font-mono text-[#2563eb] uppercase tracking-widest mb-4 block">Ecossistema Completo</span>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="showcase-card group scroll-reveal delay-100">
              <div className="w-12 h-12 rounded-full bg-purple-500/10 border border-purple-500/20 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
                <Building2Icon className="w-5 h-5 text-purple-400" />
              </div>
            ...
            <div className="scroll-reveal">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="border-l-2 border-[#2563eb] pl-6 py-2">
                  ...
                  <div className="text-sm font-mono text-white/50 uppercase">Artigos do Regimento Codificados</div>
                </div>
                <div className="border-l-2 border-[#2563eb] pl-6 py-2">
                  <div className="text-4xl font-semibold text-white mb-1">24/7</div>
                  <div className="text-sm font-mono text-white/50 uppercase">Disponibilidade Operacional</div>
                </div>
                <div className="border-l-2 border-[#2563eb] pl-6 py-2">
                  <div className="text-4xl font-semibold text-white mb-1">100%</div>
                  <div className="text-sm font-mono text-white/50 uppercase">Das escolas conectadas aprovam</div>
                </div>
              </div>
            </div>
            ...
            <div className="scroll-reveal delay-200">
              <div className="bg-[#121212] p-8 rounded-3xl border border-white/5 relative shadow-xl">
                ...
                <p className="text-white/80 font-light text-lg mb-8 leading-relaxed">
                  "O <strong className="orbitron text-white">SIGMILITAR</strong> mudou a nossa rotina. Antes perdíamos horas transcrevendo ATAs e checando pontuações no Excel. Hoje, a IA resolve a burocracia e nós focamos nos alunos. É indispensável para o modelo Cívico-Militar."
                </p>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="text-white font-medium">Gestor Escolar</div>
                    <div className="text-white/40 text-sm">Direção Geral</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="relative">
            ...
            <div className="space-y-16">
              ...
              <div className="relative flex flex-col md:flex-row items-start md:justify-between scroll-reveal">
                ...
                <div className="absolute left-0 md:left-1/2 md:-translate-x-1/2 w-12 h-12 rounded-full bg-black border-4 border-[#0A0A0A] flex items-center justify-center z-10 text-[#2563eb] font-mono font-bold text-sm shadow-[0_0_15px_rgba(37,99,235,0.2)]">01</div>
              ...
              <div className="relative flex flex-col md:flex-row items-start md:justify-between scroll-reveal">
                ...
                <div className="absolute left-0 md:left-1/2 md:-translate-x-1/2 w-12 h-12 rounded-full bg-black border-4 border-[#0A0A0A] flex items-center justify-center z-10 text-[#2563eb] font-mono font-bold text-sm shadow-[0_0_15px_rgba(37,99,235,0.2)]">02</div>
              ...
              <div className="relative flex flex-col md:flex-row items-start md:justify-between scroll-reveal">
                ...
                <div className="absolute left-0 md:left-1/2 md:-translate-x-1/2 w-12 h-12 rounded-full bg-black border-4 border-[#0A0A0A] flex items-center justify-center z-10 text-[#2563eb] font-mono font-bold text-sm shadow-[0_0_15px_rgba(37,99,235,0.2)]">03</div>
                <div className="md:w-5/12 order-3 md:order-3"></div>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-4xl mx-auto px-6 relative z-10 text-center">
          ...
          <div className="bg-black/60 backdrop-blur-xl border border-white/10 rounded-3xl p-8 md:p-12 max-w-2xl mx-auto shadow-2xl scroll-reveal">
            ...
            <form onSubmit={(e) => e.preventDefault()} className="space-y-4 text-left">
              <div>
                <label className="text-xs font-mono text-white/40 uppercase tracking-widest mb-2 block">Nome da Instituição</label>
                <input type="text" placeholder="Ex: EECM Prof. João Batista" className="w-full bg-black border border-white/10 rounded-xl px-4 py-3.5 text-white text-sm placeholder-white/30 focus:border-[#2563eb] focus:outline-none focus:ring-1 focus:ring-[#2563eb]/30 transition-colors" required />
              </div>
              <div>
                <label className="text-xs font-mono text-white/40 uppercase tracking-widest mb-2 block">E-mail Profissional</label>
                <input type="email" placeholder="gestao@escola.com.br" className="w-full bg-black border border-white/10 rounded-xl px-4 py-3.5 text-white text-sm placeholder-white/30 focus:border-[#2563eb] focus:outline-none focus:ring-1 focus:ring-[#2563eb]/30 transition-colors" required />
              </div>
              <div>
                <label className="text-xs font-mono text-white/40 uppercase tracking-widest mb-2 block">Telefone / WhatsApp</label>
                <input type="tel" placeholder="(00) 00000-0000" className="w-full bg-black border border-white/10 rounded-xl px-4 py-3.5 text-white text-sm placeholder-white/30 focus:border-[#2563eb] focus:outline-none focus:ring-1 focus:ring-[#2563eb]/30 transition-colors" required />
              </div>
              <button type="submit" className="w-full py-4 mt-4 rounded-xl text-white font-medium text-lg hover:bg-[#60a5fa] transition-colors shadow-[0_0_20px_rgba(59,130,246,0.3)] bg-[#2563eb]">
                Agendar Reunião de Implantação
              </button>
            </form>
            <p className="text-xs text-white/30 font-mono mt-6 text-center">Versão atual: 25.05.26.08:31 · Sem spam · Dados protegidos</p>
          </div>
        </div>
      </section>
            ...
            <div className="flex gap-16">
              <div>
                ...
                <ul className="space-y-2 text-sm text-white/40">
                  <li><a onClick={(e) => handleSmoothScroll(e, '#problema')} className="hover:text-white transition-colors cursor-pointer" href="#problema">Visão Geral</a></li>
                  <li><a onClick={(e) => handleSmoothScroll(e, '#modulos')} className="hover:text-white transition-colors cursor-pointer" href="#modulos">Módulos</a></li>
                  <li><a onClick={(e) => handleSmoothScroll(e, '#diferenciais')} className="hover:text-white transition-colors cursor-pointer" href="#diferenciais">Segurança</a></li>
                </ul>
              </div>
              <div>
                ...
                <ul className="space-y-2 text-sm text-white/40">
                  ...
                  <li><a onClick={(e) => handleSmoothScroll(e, '#cta')} className="hover:text-white transition-colors cursor-pointer" href="#cta">Implantação</a></li>
                  <li><Link className="hover:text-white transition-colors" href="/login">Acessar Sistema</Link></li>
                </ul>
              </div>
            </div>
          </div>
          <div className="border-t border-white/10 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
            <p className="text-white/30 text-xs font-mono">© 2026 <strong className="orbitron text-white">SIGMILITAR</strong>. Todos os direitos reservados. · v.25.05.26.08:31</p>
