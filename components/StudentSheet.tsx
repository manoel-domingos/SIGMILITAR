                  ...
                  <div className="space-y-3">
                    {allDocs.map((doc, i) => (
                      <a key={i} href={doc.url} target="_blank" rel="noopener noreferrer"
                        ...
                        <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${doc.type === 'video' ? 'bg-violet-100 dark:bg-violet-500/20 text-violet-600' : 'bg-blue-100 dark:bg-blue-500/20 text-blue-600'}`}>
                          {doc.type === 'video' ? <FileText className="w-4 h-4" /> : <Paperclip className="w-4 h-4" />}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-slate-700 dark:text-slate-200 truncate group-hover:text-blue-600">{doc.label}</p>
                          <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{new Date(doc.date).toLocaleDateString('pt-BR')}</p>
                        </div>
                        <span className="text-xs text-blue-500 shrink-0 font-medium">Abrir</span>
                      </a>
                    ))}
                  </div>
                )}
