 ...
 *   O campo de letra aceita digitação livre + sugestões (A, B, C, D…)
 *
...
export function ClassSelector({
  ...
  // ── Modo simples: select de ano + input+datalist de letra ─────────────────
  const currentGrade = value.replace(/ [A-Z]$/i, '').trim() || grades[0];
  const currentLetter = value.match(/ ([A-Z])$/i)?.[1]?.toUpperCase() || classLetters[0];
  ...
  const handleLetterChange = (newLetter: string) => {
    ...
    onChange(`${currentGrade} ${letter || currentLetter}`);
  };

  const datalistId = `class-letters-${Math.random().toString(36).slice(2, 7)}`;
    ...
    <div className="flex gap-2">
      ...
      <div className="relative w-1/3">
        <input
          ...
          placeholder="A"
          className={`w-full ${baseSelect} text-center font-bold uppercase`}
        />
